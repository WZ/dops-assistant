 ...
 * see the `dops_` prefix in masks (`dops…abcd`) so they can tell at a glance
 * which secrets in their Grafana config belong to dops-assistant.
 *
...
export interface GeneratedWebhookToken {
  ...
  tokenHash: string;
  /** First 8 chars of the random portion (after `dops_`); shown in lists alongside the masked tail */
  prefix: string;
}
 ...
 *  surprise the operator at copy-paste time. Keep it permissive enough to
 *  match human-readable labels ("grafana-prod", "Pagerduty Staging"). */
export function isValidTokenName(name: string): boolean {
  if (typeof name !== "string") return false;
  const trimmed = name.trim();
  if (trimmed.length === 0 || trimmed.length > 64) return false;
  // Allow letters, digits, space, dash, underscore, dot — covers the
  // expected set without inviting weird Unicode lookalike attacks.
