 ...
 * service. Exported for `/api/webhooks/info` so the GUI renders the actual
 * contract operators must label their alerts with —
...
export function synthesizeTestPayload(args: {
  ...
  return AlertPayloadSchema.parse({
    alerts: [{
      status: "firing",
      labels: {
        alertname: args.alertName ?? "WebhookTestAlert",
        severity: sev,
