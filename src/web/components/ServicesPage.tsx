...
import { FilterChips } from "./services/FilterChips";
import type { FilterValue } from "./services/FilterChips";
import type { ServiceListItem, ServiceHealth } from "../../types/services";
...
export function ServicesPage({
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [svcRes, hiddenRes, staleRes] = await Promise.all([
        stackFetch("/api/services"),
        stackFetch("/api/services/hidden"),
        stackFetch("/api/services/stale-unknown?days=7"),
      ]);
      if (!svcRes.ok) {
        throw new Error(`Server error: ${svcRes.status}`);
      }
      if (seq !== fetchSeqRef.current) return; // stale response
      const svcData = await svcRes.json() as { services: ServiceListItem[] };
      if (hiddenRes.ok) {
  ...
  const healthKpi = useMemo(() => {
    const counts: Record<ServiceHealth, number> = { healthy: 0, degraded: 0, down: 0, unknown: 0 };
  ...
  const availableTiers = useMemo(() => {
    ...
    for (const s of services) {
      const t = s.metadata.tags.find((tag) => tag.startsWith("tier:"));
      if (t) set.add(t.slice("tier:".length));
    }
  ...
  const availableOwners = useMemo(() => {
    const set = new Set<string>();
    for (const s of services) {
      const t = s.metadata.tags.find((tag) => tag.startsWith("owner:"));
      if (t) set.add(t.slice("owner:".length));
    }
    return Array.from(set).sort();
  }, [services]);
  ...
  const visibleServices = useMemo(() => {
    ...
    return services.filter((s) => {
      if (hiddenSet.has(s.name)) return false;
      if (q && !s.name.toLowerCase().includes(q)) return false;
      if (filters.status.length > 0 && !filters.status.includes(s.health)) return false;
      if (filters.tiers.length > 0) {
        const tier = s.metadata.tags.find((t) => t.startsWith("tier:"))?.slice("tier:".length);
        if (!tier || !filters.tiers.includes(tier)) return false;
      }
      if (filters.owners.length > 0) {
        const owner = s.metadata.tags.find((t) => t.startsWith("owner:"))?.slice("owner:".length);
    ...
    <div className="h-full overflow-y-auto px-4 py-5 relative z-[2]">
      ...
      <section aria-label="Services" className="mb-6">
        ...
        {loading ? (
          ...
          <>
            {/* Filter chips pinned under search */}
            <div className="mb-4">
              <FilterChips
                value={filters}
                onChange={setFilters}
            ...
            {sortedServices.length === 0 ? (
              <div className="rounded-lg border border-dashed border-border p-8 text-center text-muted-foreground font-mono text-[11px]">
