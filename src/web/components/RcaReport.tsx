  ...
  return (
    <div className={`rounded-xl border ${severityGlow} bg-card/50 overflow-hidden animate-fade-up noise relative`}>
      ...
      <div className="px-5 py-4 border-b border-border/30">
        ...
        {report.summary && (
          <p className="text-sm font-body text-foreground/80 leading-relaxed mt-2">
      ...
      <div className="px-5 py-4 space-y-4">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className="text-sm font-body text-foreground/90 leading-relaxed">{renderInline(report.trigger)}</p>
          </div>

          <div>
            <SectionLabel>Impact</SectionLabel>
            <p className="text-sm font-body text-foreground/90 leading-relaxed">{renderInline(report.impact.description)}</p>
            ...
            <div className="border-l-2 border-primary/20 pl-4 space-y-2.5 ml-1">
              {report.timeline.map((evt, i) => (
                <div key={i} className="flex items-start gap-2.5 animate-fade-up" style={{ animationDelay: `${i * 0.04}s` }}>
                  ...
                  <span className="text-sm font-body text-foreground/90 leading-relaxed">{renderInline(evt.event)}</span>
                </div>
              ))}
            </div>
          </Section>
          ...
          <Section label="Contributing Factors" count={report.contributingFactors.length}>
            <ul className="space-y-1.5 ml-1">
              {report.contributingFactors.map((f, i) => (
                <li key={i} className="flex items-start gap-2 text-sm font-body text-foreground/90 leading-relaxed">
          ...
          <Section label="Recommended Actions" count={report.recommendedActions.length}>
            <ol className="space-y-2 ml-1">
              {report.recommendedActions.map((a, i) => (
                <li
                  key={i}
                  className="flex items-start gap-2 text-sm font-body text-foreground/90 leading-relaxed animate-fade-up"
